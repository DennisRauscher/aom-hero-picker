...
import { ChevronRight, ChevronLeft, Swords, Shield, Coins, Anchor, Map, X } from 'lucide-react'
...
export function AgeOfMythologyChooser() {
  ...
  const [showLegalInfo, setShowLegalInfo] = useState<'impressum' | 'privacy' | 'terms' | null>(null)

  useEffect(() => {
    const godName = searchParams.get('god')
    if (godName) {
      const god = gods.find(g => g.name.toLowerCase() === godName.toLowerCase())
      if (god) {
        setResult(god.name)
        setSelectedGod(god)
      }
  ...
  const handleAnswer = (answer: Answer) => {
    ...
    setTimeout(() => {
      if (currentQuestion < questions.length - 1) {
        ...
        const sortedGods = Object.entries(newScores).sort((a, b) => b[1] - a[1])
  ...
  const renderGodCircles = (godIds: number[], title: string) => (
    <div className="mt-4">
      <h4 className="text-lg sm:text-xl font-semibold mb-2 text-yellow-300">{title}:</h4>
...
<div className="min-h-screen bg-gradient-to-br from-indigo-900 to-purple-900 flex flex-col items-center justify-center p-2 sm:p-4">
      <div className="w-full max-w-4xl bg-black bg-opacity-50 backdrop-blur-lg rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden">
        <div className="p-4 sm:p-8">
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-center mb-6 sm:mb-8 text-yellow-400 tracking-wide">AoM: God Chooser</h1>
          <AnimatePresence mode="wait">
            {result && selectedGod ? (
              ...
              >
                <h2 className="text-2xl sm:text-3xl font-bold mb-4 text-yellow-300">Your Chosen God is:</h2>
                <p className="text-4xl sm:text-5xl md:text-6xl font-bold mb-6 sm:mb-8 text-yellow-400 drop-shadow-glow">{result}</p>
                <div className="mb-6 sm:mb-8 flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-8">
                  ...
                  >
                    <Image
                      src={selectedGod.artwork ? "/" + selectedGod.artwork : '/placeholder.png'}
                  ...
                  <div className="text-left max-w-sm sm:max-w-none">
                    <h3 className="text-xl sm:text-2xl font-semibold mb-2 text-green-400">Strengths:</h3>
                    ...
                    <h3 className="text-xl sm:text-2xl font-semibold mt-4 mb-2 text-red-400">Weaknesses:</h3>
                    <ul className="list-none space-y-1 sm:space-y-2">
                    ...
                    {renderGodCircles(selectedGod.counteredBy, "Gods That Counter You")}
                  </div>
                </div>
                ...
                {secondBestGod && (
                  <div className="mt-6 sm:mt-8">
                    <h3 className="text-xl sm:text-2xl font-bold mb-4 text-yellow-300">Second Best Option:</h3>
                    <div className="flex items-center justify-center">
                      ...
                      >
                        <Image
                          src={secondBestGod.artwork ? "/" + secondBestGod.artwork : '/placeholder.png'}
                          alt={secondBestGod.name}
                          layout="fill"
                          objectFit="cover"
                        />
                      </motion.div>
                      <p className="text-2xl sm:text-3xl font-bold text-yellow-400">{secondBestGod.name}</p>
                    </div>
                  </div>
                )}
                <motion.button
              ...
              >
                <div className="flex items-center justify-center mb-6 sm:mb-8">
                  ...
                  <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-white">{questions[currentQuestion].question}</h2>
                </div>
